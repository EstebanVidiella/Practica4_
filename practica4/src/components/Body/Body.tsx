...
const Body: FC<bodyProps> = (props) => {
  ...
  const [url, setUrl] = useState<string>(
    props.type
      ? "http://openlibrary.org/search.json?title=" + props.filter
      : "http://openlibrary.org/search.json?author=" + props.filter
  );
      ...
      if (data.num_found - data.start > 100) {
        ...
        if (data.start < data.num_found) {
          ...
          var aux1: Libros[] = docs.sort((a, b) => {
            if (
              a.title.charAt(0).toLocaleLowerCase >
              b.title.charAt(0).toLocaleLowerCase
            ) {
              return -1;
            }
            if (
    ...
    <div className="bodyContainer">
      {!finish && (
        <div className="loader">
          <div className="boxInfo">Un momento por favor...</div>
        </div>
      )}
      {finish && (
        <div>
            ...
            {typeShow === "list" && (
              <div>
                <div className="sortOption">
                  {sortBy === "alpha" && "*"}
                  <div className="sort" onClick={(e) => setSortBy("alpha")}>
                    Orden Alfabetico
                  </div>
                  {sortBy === "year" && "*"}
                  <div className="sort" onClick={(e) => setSortBy("year")}>
                    Orden Anual
                  </div>
                </div>
                <div className="botones">
                  <div onClick={(e) => updateIndexLess()} className="btn-grad2">
                    Anterior
                  </div>
                  <div onClick={(e) => updateIndex()} className="btn-grad2">
                    Siguiente
                  </div>
                </div>
                <div>
                  {sortBy === "alpha" &&
                    sortAlpha.map((book, index) => {
                      if (index >= minIndex && index < maxIndex) {
                        return (
                          <div className="List">
                            <div className="texto_title">
                              Titulo: {book.title}
                            </div>
                            <div className="texto_title">
                        ...
                        <div className="box">
                          ...
                          {book.cover_i && (
                            <div
                              style={{
                                backgroundImage:
                                  "url(http://covers.openlibrary.org/b/id/" +
                                  book.cover_i +
                                  "-M.jpg",
                                height: "200px",
                                width: "180px",
                          ...
                          {book.id_amazon && (
                            <a
                              className="texto_amz"
                              href={
                                "https://www.amazon.es/dp/" + book.id_amazon[0]
                              }
                            >
                              Comprar aqui
                            </a>
